...
public class CTMSpriteResolver {
    ...
    public static CTMSpriteResolver from(Function<Material, TextureAtlasSprite> spriteGetter, ResourceLocation modelId) {
        CTMSpriteResolver holder = new CTMSpriteResolver();
        holder.missingResult = new SpriteResultHolder(spriteGetter.apply(new Material(InventoryMenu.BLOCK_ATLAS, MissingTextureAtlasSprite.getLocation())), true);
        ...
        CTMJsonHelper.getCTMTextureOverrides(modelId).forEach((key, value) -> {
            ...
            boolean missing = to == holder.missingResult.get();

            // If the key is missing we don't add the entry
            // As we don't want to have "null -> sprite"
            if (from != holder.missingResult.get())
                holder.sprites.put(from, new SpriteResultHolder(to, missing));
        });
